...
/* TS and TSX */
const filesTS = glob
  .sync('**/*.ts', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
const filesTSX = glob
  .sync('**/*.tsx', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
if (filesTSX.length || filesTS.length) runPrettier([...filesTSX, ...filesTS]);

/* Javascript */
const filesJS = glob
  .sync('**/*.js', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
if (filesJS.length) runPrettier(filesJS);

/* YAML */
const filesYML = glob
  .sync('**/*.yml', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
if (filesYML.length) runPrettier(filesYML);

/* MARKDOWN */
const filesMD = glob
  .sync('**/*.md', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
const filesMDX = glob
  .sync('**/*.mdx', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
if (filesMD.length || filesMDX.length) runPrettier([...filesMD, ...filesMDX]);

/* JSON */
const filesJSON = glob
  .sync('**/*.json', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
if (filesJSON.length) runPrettier(filesJSON);

/* LESS */
const filesLESS = glob
  .sync('**/*.less', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
if (filesLESS.length) runPrettier(filesLESS);

/* SCSS */
const filesSCSS = glob
  .sync('**/*.scss', { ignore: filesToIgnore })
  .filter((f) => !onlyChanged || changedFiles.has(f));
